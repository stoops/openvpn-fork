...
bool
multi_tcp_process_outgoing_link(struct multi_context *m, bool defer, const unsigned int mpp_flags, int t)
{
    ...
    if (mi)
    {
        ret = multi_process_outgoing_link_dowork(m, mi, mpp_flags, t);
        if (!ret)
        {
            mi = NULL;
        }
