...
write_control_auth(struct tls_multi *multi, struct tls_session *session, struct key_state *ks, struct buffer *buf, struct link_socket_actual **to_link_addr, int opcode, int max_ack, bool prepend_ack)
{
    ...
    uint8_t header = kx->key_id | (opcode << P_OPCODE_SHIFT);
    ...
    ASSERT(reliable_ack_write(kx->rec_ack, kx->lru_acks, buf, &kw->session_id_remote, max_ack,
                              prepend_ack));
...
bool
read_control_auth(struct tls_multi *multi, struct buffer *buf, struct tls_wrap_ctx *ctx,
