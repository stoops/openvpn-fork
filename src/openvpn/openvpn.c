...
void *threaded_buffer_bloat_buster(void *args)
{
    ...
    if ((strcmp(cadr, "0.0.0.0") != 0) && (strcmp(cadr, "255.255.255.255") != 0))
    {
        ...
        strncpy(adrl[vlen], cadr, MAX_STRLENG-8);
        ++vlen;
    }
    ...
    while (true)
    {
                ...
                while ((he = hash_iterator_next(&hi)))
                {
                    ...
                    if (multi_route_defined(m, mr))
                    {
                        ...
                        if (vlen < maxc)
                        {
                            ...
                            adrs[vlen].sin_family = AF_INET;
                            bzero(adrl[vlen], MAX_STRLENG * sizeof(char));
                            strncpy(adrl[vlen], madr, MAX_STRLENG-8);
                            ++vlen;
                        }
                    }
                    gc_free(&gc);
        ...
        if (vlen > 0) {
            ...
            msg(D_LOG_RW, "%s BUST sent [%ld][%ld] [%d][%d] [%d] [%s][%d]", (p->m) ? "TCPv4_SERVER" : "TCPv4_CLIENT", secs, last, indx, vlen, wsec, adrl[indx], sent);
        } else {
            sleep(1);
        }
    }
...
int proc_addr_cons_maps(struct context *d, struct mtio_args *args, struct mtio_cons *cons)
{
    ...
    struct buffer *bufp = &d->c2.buffers->read_tun_bufs[TUN_BAT_MAX-1];
    if ((d->options.ce.mtio_mode) && (mroute_extract_addr_ip(&srca, &dsta, bufp) == MROUTE_EXTRACT_SUCCEEDED))
    {
        int flag = 1;
        ...
        if (flag == 1)
        {
            ...
            if (logs == 1)
            {
                ...
                msg(M_INFO, "STATE MTIO maps <%d>(%d) [%s][%s] {%d}{%d}", indx, expr, a, b, thid, cons[indx].thid);
            }
            thid = cons[indx].thid;
            cons[indx].last = secs;
        }
