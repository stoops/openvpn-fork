...
void *threaded_buffer_bloat_buster(void *a)
{
    ...
    while (true)
    {
        ...
        if (vlen > 0)
        {
            ...
            msg(D_LOG_RW, "%s BUST data [%ld][%ld] [%d][%d] [%d] [%s][%d]", (p->m) ? "TCPv4_SERVER" : "TCPv4_CLIENT", secs, last, indx, vlen, wsec, adrl[indx], sent);
        }
        else { sleep(1); }
    }
...
void *tunnel_point_to_point(void *a)
{
    ...
    while (true)
    {
        ...
        {
            process_io(c, c->c2.link_sockets[0], b, 3);
        }
        P2P_CHECK_SIG();
    }
...
void *threaded_io_management(void *a)
{
    ...
    ssize_t leng = 0;
    uint8_t *ptra;
    uint8_t busy[maxx], proc[maxx], load[maxx], sels[maxx];
    fd_set rfds;
    struct timeval timo;
