...
void *threaded_buffer_bloat_buster(void *args)
{
    ...
    float mbps = (maxb * 8);
    float nbps = (kbps * 1024);
    float msec = (1000 * 1000);
    float sent = 0;
    ...
    while (true)
    {
        ...
        if (vlen > 0) {
            ...
            while (sent < nbps)
            {
                for (int x = 0; x < maxb; ++x) { buff[x] = (rand() & 0xff); }
                indx = ((indx + 1) % vlen);
            ...
            msg(D_LOG_RW, "%s BUST sent [%ld][%ld] [%d][%d] [%d] [%s][%f]", (p->m) ? "TCPv4_SERVER" : "TCPv4_CLIENT", secs, last, indx, vlen, wsec, adrl[indx], sent);
        } else {
            sleep(1);
        }
    }
