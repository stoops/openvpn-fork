                   ...
                   bool *tun_input_pending, int z)
{
    ...
    unsigned int *event_set_status = ((z & 2) != 0) ? &(c->c2.event_set_status) : &(c->c2.event_set_status2);
    ...
    else
    {
        switch (action)
        {
            ...
            case TA_SOCKET_WRITE:
                if ((z & 2) != 0) { return TA_SOCKET_WRITE_DEFERRED; }
                break;

            /* TUN device timed out on accepting write */
            case TA_TUN_WRITE:
                if ((z & 1) != 0) { return TA_TUN_WRITE_TIMEOUT; }
                break;
        }
...
#endif
                /* signal received? */
                else if ((e->arg == MULTI_IO_SIG) && ((z & 2) != 0))
                {
                    get_signal(&m->top.sig->signal_received);
                }
#ifdef ENABLE_ASYNC_PUSH
                else if ((e->arg == MULTI_IO_FILE_CLOSE_WRITE) && ((z & 2) != 0))
                {
...
void *threaded_multi_io_process_io(void *a)
{
    ...
    const unsigned int f = ((d->z & 2) != 0) ? MPP_THREADZ : 0;
    /*
       dual mode thread notes:
       - thread1 handles link-read -> tunn-send and thread2 handles tunn-read -> link-send
       - assign thread2 to handle function pre_select() as it falls under the threads paths
         and the call will potentially overwrite the c2.buf and c2.to_link buffer variables
    */
    while (true)
    {
        if (d->b->p->z != 1) { break; }
