...
process_outgoing_link(struct context *c, struct link_socket *sock)
{
    ...
    if (c->c2.buf.len < 1 && c->c2.to_link.len < 1 && REKEY_OUT(c))
    {
...
void *threaded_process_io(void *a)
{
    /*
        dual mode commit notes:
          ...
          - this mode is built on top of the organization and separation code mods previously implemented in bulk mode
    */
