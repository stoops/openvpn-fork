...
encrypt_sign(struct context *c, bool comp_frag)
{
    ...
    if (c->c2.tls_multi->plaintext_write_buf.len)
    {
        op_v = 2;
        c->c2.buf = c->c2.tls_multi->plaintext_write_buf;
        c->c2.tls_multi->plaintext_write_buf.len = 0;
    }
...
void *threaded_process_io(void *a)
{
    /*
        dual mode commit notes:
          ...
          - dual mode is built on the organization and separation code previously implemented in bulk mode
    */
