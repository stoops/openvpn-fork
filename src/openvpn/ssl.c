...
tls_multi_init_finalize(struct tls_multi *multi, int tls_mtu)
{
    ...
    tls_session_init(multi, &multi->session[TM_THREADZ]);
}
                  ...
                  struct link_socket_info *to_link_socket_info, interval_t *wakeup)
{
    ...
    if (TLS_AUTHENTICATED(multi, &multi->session[TM_INITIAL].key[KS_PRIMARY]))
    {
        ...
        if (multi->opt.dual_mode && (ks->state == S_GENERATED_KEYS && ks->authenticated == KS_AUTH_TRUE))
        {
            move_auth_sess_name = TM_THREADZ;
        }
        ...
        if (move_auth_sess_name == TM_THREADZ)
        {
            struct key_state *ks = get_key_scan(multi, TM_THREADZ);
            ks->keys_idno = 3;
            msg(M_INFO, "%s DUAL keys (%d) [%d][%d] {%d}", (multi->opt.mode == MODE_SERVER) ? "TCPv4_SERVER" : "TCPv4_CLIENT", TM_THREADZ, ks->key_id, ks->keys_idno, multi->multi_state);
        }
    }
                ...
                struct crypto_options **opt, bool floated, const uint8_t **ad_start)
{
    ...
    else
    {
         ...
         */
        if (i != TM_ACTIVE && i != TM_INITIAL && i != TM_THREADZ)
        {
...
tls_select_encryption_key(struct tls_multi *multi)
{
    ...
    for (int i = (KEY_SCAN_SIZE - 1); i > -1; --i)
    {
        struct key_state *ks = get_key_scan(multi, i);
