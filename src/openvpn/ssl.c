...
{
    const struct key_state *ks = &session->key[KS_MAIN];
    size_t overhead = 0;
 ...
 */
bool
key_state_soft_reset(struct tls_multi *multi)
{
            ...
            interval_t *wakeup)
{
    ...
    if (!multi->gens_stat && !multi->reno_stat && !ks->keys_lame && ks->state >= S_GENERATED_KEYS && should_trigger_renegotiation(session, ks))
    {
        msg(D_TLS_DEBUG_LOW,
