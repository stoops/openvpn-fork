 ...
 */
bool multi_process_incoming_link(struct multi_context *m, const unsigned int mpp_flags, struct link_socket *sock);
 ...
 */
static inline bool
multi_process_outgoing_tun(struct multi_context *m, const unsigned int mpp_flags, int t)
{
    bool ret = true;
...
static inline void
multi_tcp_process_incoming_link(struct multi_context *m, unsigned int mpp_flags, int t)
{
    struct multi_instance *mi = m->pending2;
    if (!mi) { return; }
    ...
    {
        multi_process_incoming_link(m, mpp_flags, mi->context.c2.link_sockets[0]);
    }
    if (!IS_SIG(&mi->context))
    {
        multi_process_outgoing_tun(m, mpp_flags, t);
    }
    mi->post[t] = true;
    m->post[t] = true;
}
...
static inline bool
multi_process_outgoing_link_dowork(struct multi_context *m, struct multi_instance *mi, const unsigned int mpp_flags, int t)
{
    bool ret = true;
    set_prefix(mi);
