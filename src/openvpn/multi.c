...
multi_process_post(struct multi_context *m, struct multi_instance *mi, const unsigned int flags)
{
    ...
    if (IS_SIG(&mi->context))
    {
        if ((flags & MPP_CLOSE_ON_SIGNAL) && ((flags & MPP_THREAD_RLWT) != 0))
        {
    ...
    else
    {
        ...
        if ((flags & MPP_THREAD_RTWL) != 0) { multi_set_pending(m, LINK_OUT(c) ? mi : NULL); }
        if ((flags & MPP_THREAD_RLWT) != 0) { multi_set_pending2(m, TUN_OUT(d) ? mi : NULL); }
    }
 ...
 */
bool
multi_process_incoming_link(struct multi_context *m, struct multi_instance *instance, const unsigned int mpp_flags, struct link_socket *sock)
{
    struct gc_arena gc = gc_new();
