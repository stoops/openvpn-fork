...
struct multi_instance *multi_learn_peer_addr(struct multi_context *m, struct mroute_addr *p)
{
    struct multi_instance *i = m->pending, *r = NULL;
    struct multi_link *l;
    in_addr_t a = ntohl(p->v4.addr);
    for (int x = 0; x < m->mtio_info.maxc; ++x)
    {
        ...
        if ((strcmp(l->usrs, "") != 0) && (l->ladr == a))
        {
            if (i)
            {
                multi_context_switch_addr(m, i);
                r = i;
            }
            else
            {
                for (int y = 0; y < m->max_clients; ++y)
                {
                    struct multi_instance *t = m->instances[y];
                    if (t && (strcmp(t->mtio_addr.usrs, l->usrs) == 0))
                    {
                        multi_context_switch_addr(m, t);
                    }
                }
            }
        }
    ...
    if (m->pending)
    {
        ...
        if (BLEN(&c->c2.buf2) > 0)
        {
            ...
            if (TUNNEL_TYPE(m->top.c1.tuntap) == DEV_TYPE_TUN)
            {
                ...
                else if ((multi_get_instance_by_virtual_addr(m, &src, true) != m->pending) && (multi_learn_peer_addr(m, &src) != m->pending))
                {
...
void threaded_tunnel_server(struct context *c, struct context *d)
{
    ...
    if (c->options.ce.bust_mode)
    {
        ...
        pthread_create(&(thrb), NULL, threaded_buffer_bloat_buster, &(a[0]));
    }
